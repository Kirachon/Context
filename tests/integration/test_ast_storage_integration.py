...
'''
                    ...
                    parse_result = self.parser.parse(
                        Path("user_manager.py"), python_code
                    )
                    ...
                    if parse_result.parse_success and parse_result.ast_root:
                        storage_result = await self.ast_store.store_parse_result(
                            parse_result
                        )

                        # Verify storage calls were made
                        assert (
                            mock_client.upsert.call_count >= 0
                        )  # May be 0 if no symbols extracted
                    ...
                    print(
                        f"Pipeline test completed - Parse success: {parse_result.parse_success}"
                    )
    ...
    async def test_search_filtering(self, mock_get_client):
        ...
        with patch.object(
            ...
            for request in test_cases:
                ...
                assert (
                    "query_filter" in call_args.kwargs
                    or call_args.kwargs.get("query_filter") is None
                )
...
'''
    ...
    def test_performance_characteristics(self):
        ...
        request = ASTSearchRequest(
            query="test function", limit=10, symbol_types=[SymbolType.FUNCTION]
        )
