...
class TestIncrementalIndexing:
    ...
    @patch('src.indexing.models.update_file_metadata')
    @patch('src.indexing.models.get_file_metadata')
    async def test_incremental_updates(self, mock_get, mock_update, mock_create, temp_dir):
...
class TestFileMonitorIntegration:
    ...
    @patch('src.indexing.models.create_file_metadata')
    @patch('src.indexing.models.get_file_metadata')
    async def test_monitor_queues_changes(self, mock_get, mock_create, mock_observer_class, temp_dir):
...
class TestMCPIndexingTools:
    ...
    async def test_start_monitoring_tool(self, mock_start):
        ...
        start_monitoring = next((t for t in registered_tools if t.__name__ == 'start_monitoring'), None)
    ...
    async def test_indexing_status_tool(self):
        ...
        indexing_status = next((t for t in registered_tools if t.__name__ == 'indexing_status'), None)
...
class TestPerformance:
    ...
    async def test_indexing_throughput(self, mock_get, mock_create, temp_dir):
        ...
        assert duration < 6.0, f"Indexing took {duration}s, should be < 6s for {num_files} files"
        assert throughput > 1000, f"Throughput {throughput:.0f} files/min, should be > 1000"
