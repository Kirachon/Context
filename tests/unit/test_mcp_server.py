...
class TestMCPConnectionLifecycle:
    ...
    async def test_reconnection_logic(self, mcp_server_instance):
        ...
        with patch.object(
            mcp_server_instance, "_attempt_reconnection", new_callable=AsyncMock
        ) as mock_reconnect:
...
class TestMCPToolRegistration:
    ...
    def test_register_tools_success(
        self, mock_fastmcp, mock_cap_tools, mock_health_tools, mcp_server_instance
    ):
...
class TestMCPServerStartup:
    ...
    async def test_start_success(
        self, mock_signals, mock_register, mock_create, mcp_server_instance
    ):
...
class TestMCPServerShutdown:
    ...
    async def test_shutdown_with_error(self, mcp_server_instance):
        ...
        with patch.object(
            mcp_server_instance.shutdown_event,
            "set",
...
class TestMCPGlobalFunctions:
    ...
    async def test_start_mcp_server_function(self):
        ...
        with patch(
            "src.mcp_server.mcp_app.mcp_server.start", new_callable=AsyncMock
        ) as mock_start:
    ...
    async def test_shutdown_mcp_server_function(self):
        ...
        with patch(
            "src.mcp_server.mcp_app.mcp_server.shutdown", new_callable=AsyncMock
        ) as mock_shutdown:
