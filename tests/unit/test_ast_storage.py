...
from src.search.ast_models import (
    ASTSearchRequest, SymbolType, SearchScope,
    SymbolEmbeddingPayload, ClassEmbeddingPayload, ImportEmbeddingPayload
)
from src.search.ast_search import ASTSearchService
from src.indexing.ast_indexer import ASTIndexer
from src.parsing.models import (
    ParseResult, Language, SymbolInfo, ClassInfo, ImportInfo, ParameterInfo
)
...
class TestASTVectorStore:
    ...
    def test_generate_class_search_text(self):
        ...
        search_text = self.ast_store._generate_class_search_text(class_info, parse_result)
    ...
    def test_generate_import_search_text(self):
        ...
        search_text = self.ast_store._generate_import_search_text(import_info, parse_result)
    ...
    def test_generate_unique_ids(self):
        ...
        # Test symbol ID
        symbol = SymbolInfo(name="test_func", type="function", line_start=10, line_end=15)
    ...
    async def test_ensure_collections(self, mock_get_client):
        ...
        assert mock_client.create_collection.call_count == 3  # symbols, classes, imports
        ...
        with patch.object(self.ast_store.embedding_service, 'initialize', new_callable=AsyncMock):
            with patch.object(self.ast_store.embedding_service, 'embedding_dim', 384):
                with patch.object(self.ast_store.embedding_service, 'generate_embedding', 
                    ...
                    # Create test parse result
                    symbol = SymbolInfo(name="test_func", type="function", line_start=1, line_end=5)
