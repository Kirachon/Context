...
async def test_enqueue_and_process_changed_file(tmp_path: Path, monkeypatch):
    ...
    idx = IncrementalIndexer(
        batch_window_ms=100, embedding_service=svc, vector_store=store
    )
    idx.set_index_ops(
        index_file_func=AsyncMock(return_value={}),
        remove_file_func=AsyncMock(return_value=True),
    )
    await idx.start()
    ...
    assert any(
        any(v.get("id").endswith("a.py") for v in batch) for batch in store.upserts
    )
...
async def test_no_redundant_processing_same_hash(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr("src.realtime.incremental_indexer.get_parser", lambda: parser)
    idx = IncrementalIndexer(
        batch_window_ms=50, embedding_service=svc, vector_store=store
    )
    idx.set_index_ops(
        index_file_func=AsyncMock(return_value={}),
        remove_file_func=AsyncMock(return_value=True),
...
async def test_batching_multiple_files(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr("src.realtime.incremental_indexer.get_parser", lambda: parser)
    idx = IncrementalIndexer(
        batch_window_ms=200, embedding_service=svc, vector_store=store
    )
    idx.set_index_ops(
        index_file_func=AsyncMock(return_value={}),
        remove_file_func=AsyncMock(return_value=True),
...
async def test_removed_file_calls_remove(tmp_path: Path, monkeypatch):
    ...
    idx.set_index_ops(
        index_file_func=AsyncMock(return_value={}), remove_file_func=mock_remove
    )
    await idx.start()
    # Delete then enqueue
    f.unlink()
...
async def test_dependents_skip_unsupported(tmp_path: Path, monkeypatch):
    ...
    parser = FakeParser(
        {str(b): [ImportInfo(module=str(a.with_suffix("")))], str(a): []}
    )
    monkeypatch.setattr("src.realtime.incremental_indexer.get_parser", lambda: parser)
    idx = IncrementalIndexer(
        batch_window_ms=50,
