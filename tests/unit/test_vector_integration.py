...
class TestQdrantClient:
    ...
    async def test_connect_success(self, mock_qdrant_class, qdrant_service):
        ...
        # Mock health check
        with patch.object(qdrant_service, 'health_check', return_value={"status": "healthy"}):
    ...
    async def test_connect_failure(self, qdrant_service):
        ...
        with patch('src.vector_db.qdrant_client.QdrantClient', side_effect=Exception("Connection failed")):
            result = await qdrant_service.connect()
...
class TestIntegration:
    ...
    async def test_file_indexing_with_vectors(self, mock_generate_embedding, mock_get_client):
        ...
        with patch('builtins.open', mock_open(read_data="print('hello')")):
            with patch('src.indexing.models.create_file_metadata'):
                with patch('src.indexing.models.get_file_metadata', return_value=None):
                    with patch('src.vector_db.vector_store.upsert_vector', return_value=True) as mock_upsert:
