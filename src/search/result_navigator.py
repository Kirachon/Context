...
from src.search.result_presenter import FormattedResult
...
class SearchResultNavigator:
    ...
    def filter_results(
        ...
        filter_options: NavigationFilter
    ) -> List[SearchResult]:
        ...
        if filter_options.file_types:
            filtered = [
                r for r in filtered
                if r.file_path.suffix.lower() in [
        ...
        if filter_options.directories:
            filtered = [
                r for r in filtered
                if any(str(r.file_path).startswith(d) for d in filter_options.directories)
            ]
        ...
        if filter_options.score_threshold is not None:
            filtered = [
                r for r in filtered
                if r.score >= filter_options.score_threshold
            ]
        ...
        if filter_options.content_pattern:
            ...
            filtered = [
                r for r in filtered
                if r.content and pattern in r.content.lower()
            ]
    ...
    def find_related_results(
        ...
        same_dir = [
            r for r in self.current_results
            if r.file_path.parent == result.file_path.parent and r != result and r not in related
        ]
        ...
        similar_score = [
            r for r in self.current_results
            if abs(r.score - result.score) <= score_range and r != result and r not in related
        ]
    ...
    def get_navigation_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "current_result": str(self.current_results[self.current_index].file_path) if self.current_results else None,
