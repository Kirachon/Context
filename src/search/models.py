...
class SearchRequest(BaseModel):
    ...
    file_types: Optional[List[str]] = Field(
        default=None, description="Filter by file extensions (e.g., ['.py', '.js'])"
    )
    directories: Optional[List[str]] = Field(
        default=None, description="Filter by directory paths"
    )
    exclude_patterns: Optional[List[str]] = Field(
        default=None, description="Exclude patterns (e.g., ['test', '__pycache__'])"
    )
    ...
    authors: Optional[List[str]] = Field(
        default=None, description="Filter by authors (if metadata available)"
    )
    modified_after: Optional[str] = Field(
        default=None, description="ISO 8601 modified time lower bound"
    )
    modified_before: Optional[str] = Field(
        default=None, description="ISO 8601 modified time upper bound"
    )
...
class SearchResult(BaseModel):
    ...
    line_numbers: Optional[List[int]] = Field(
        default=None, description="Relevant line numbers"
    )
...
class SearchStats(BaseModel):
    ...
    total_searches: int = Field(
        ..., ge=0, description="Total number of searches performed"
    )
...
class SearchError(BaseModel):
    ...
    query: Optional[str] = Field(
        default=None, description="Query that caused the error"
    )
