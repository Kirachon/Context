...
class CollectionManager:
    ...
    async def create_collection(self, name: str, vector_size: int = None, distance: models.Distance = None) -> bool:
        """
        ...
        try:
            ...
            logger.info(f"Creating collection: {name} (size: {vector_size}, distance: {distance.value})")
    ...
    async def get_collection_stats(self, name: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"Error getting collection stats for {name}: {e}", exc_info=True)
            return None
