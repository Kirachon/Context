...
class QdrantClientService:
    ...
    def __init__(self):
        ...
        logger.info(
            f"QdrantClientService initialized for {settings.qdrant_host}:{settings.qdrant_port}"
        )
        ...
        for attempt in range(1, self.max_retries + 1):
            try:
                ...
                self.client = QdrantClient(
                    ...
                    api_key=(
                        settings.qdrant_api_key if settings.qdrant_api_key else None
                    ),
                    timeout=10.0,
                )
                ...
                if health.get("status") == "healthy":
                    ...
                    logger.info(
                        f"Successfully connected to Qdrant (attempt {attempt}/{self.max_retries})"
                    )
                    return True
                else:
                    logger.warning(
                        f"Qdrant health check failed (attempt {attempt}/{self.max_retries})"
                    )
