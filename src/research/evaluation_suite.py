...
""",
    ...
    def run_embedding_comparison(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Sentence-transformers evaluation complete: F1={st_results.f1_score:.3f}"
            )
        except Exception as e:
        ...
        try:
            ...
            logger.info(
                f"UniXcoder evaluation complete: F1={unixcoder_results.f1_score:.3f}"
            )
        except Exception as e:
        ...
        results["recommendations"] = self._generate_recommendations(
            results["models"], results["comparison"]
        )
    ...
    def run_query_pattern_evaluation(self) -> Dict[str, Any]:
        ...
        for sample in self.embedding_evaluator.code_samples:
            if sample.parse_result and sample.parse_result.parse_success:
                ...
                for pattern in patterns:
                    ...
                    try:
                        matches = self.query_engine.execute_query(
                            pattern, sample.code, sample.id
                        )
                        ...
                        results["pattern_effectiveness"][pattern.name]["languages"].add(
                            sample.language
                        )
                        ...
                        logger.warning(
                            f"Pattern {pattern.name} failed on {sample.id}: {e}"
                        )
        ...
        for pattern_name, effectiveness in results["pattern_effectiveness"].items():
            ...
            effectiveness["success_rate"] = effectiveness["successful_matches"] / max(
                effectiveness["total_tests"], 1
            )

        # Calculate overall success rate
        results["overall_success_rate"] = results["successful_matches"] / max(
            results["patterns_tested"], 1
        )
        ...
        if all(comparison["availability"].values()):
            ...
            comparison["performance"] = {
                "encoding_time": {
                    ...
                    "winner": (
                        ...
                        if st_results["encoding_time_ms"]
                        < ux_results["encoding_time_ms"]
                        else "unixcoder"
                    ),
                },
                "memory_usage": {
                    ...
                    "winner": (
                        "sentence_transformers"
                        if st_results["memory_usage_mb"] < ux_results["memory_usage_mb"]
                        else "unixcoder"
                    ),
                },
            }
            ...
            comparison["accuracy"] = {
                "f1_score": {
                    ...
                    "winner": (
                        "sentence_transformers"
                        if st_results["f1_score"] > ux_results["f1_score"]
                        else "unixcoder"
                    ),
                },
                "similarity_accuracy": {
                    ...
                    "winner": (
                        "sentence_transformers"
                        if st_results["avg_similarity_accuracy"]
                        > ux_results["avg_similarity_accuracy"]
                        else "unixcoder"
                    ),
                },
                "cross_language_accuracy": {
                    ...
                    "winner": (
                        "sentence_transformers"
                        if st_results["cross_language_accuracy"]
                        > ux_results["cross_language_accuracy"]
                        else "unixcoder"
                    ),
                },
            }
    ...
    def _generate_recommendations(
        self, models: Dict[str, Any], comparison: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        if not comparison["availability"].get("unixcoder", False):
            ...
            recommendations["reasoning"].append(
                "UniXcoder not available or failed to load"
            )
            recommendations["next_steps"].append(
                "Investigate UniXcoder installation requirements"
            )
        ...
        if all(comparison["availability"].values()):
            ...
            if st_better_count > ux_better_count:
                recommendations["primary_recommendation"] = "keep_current"
                recommendations["reasoning"].append(
                    f"Sentence-transformers wins {st_better_count}/{st_better_count + ux_better_count} metrics"
                )
            elif ux_better_count > st_better_count:
                recommendations["primary_recommendation"] = "switch_to_unixcoder"
                recommendations["reasoning"].append(
                    f"UniXcoder wins {ux_better_count}/{st_better_count + ux_better_count} metrics"
                )
            else:
                recommendations["primary_recommendation"] = "keep_current"
                recommendations["reasoning"].append(
                    "Models perform similarly, keep current for stability"
                )
    ...
    def run_complete_evaluation(self) -> Dict[str, Any]:
        ...
        final_results = {
            ...
            "executive_summary": self._generate_executive_summary(
                embedding_results, query_results
            ),
            "technical_details": {
                "dataset_size": len(self.embedding_evaluator.code_samples),
                "languages_tested": len(
                    set(s.language for s in self.embedding_evaluator.code_samples)
                ),
                "similarity_pairs": len(self.embedding_evaluator.similarity_pairs),
                "query_patterns": sum(
                    len(patterns)
                    for patterns in self.query_engine.get_all_patterns().values()
                ),
            },
        }
    ...
    def _generate_executive_summary(
        self, embedding_results: Dict[str, Any], query_results: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        if (
            "unixcoder" in embedding_results["models"]
            and "error" not in embedding_results["models"]["unixcoder"]
        ):
            summary["key_findings"].append(
                "UniXcoder successfully evaluated and compared"
            )
            summary["recommendations"]["embedding_model"] = embedding_results[
                "recommendations"
            ]["primary_recommendation"]
        else:
            summary["key_findings"].append(
                "UniXcoder not available - keeping current model"
            )
        ...
        if query_results["overall_success_rate"] > 0.5:
            summary["key_findings"].append(
                f"Tree-sitter queries successful ({query_results['overall_success_rate']:.1%} success rate)"
            )
        ...
        summary["success_metrics"] = {
            "embedding_evaluation_completed": "unixcoder"
            in embedding_results["models"],
            "query_patterns_tested": query_results["patterns_tested"],
            "cross_language_similarity_evaluated": embedding_results[
                "dataset_stats"
            ].get("cross_language_pairs", 0)
            > 0,
