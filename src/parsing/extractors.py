...
class PythonExtractor(BaseExtractor):
    ...
    def _extract_python_class(
        self, node: ASTNode, classes: List[ClassInfo], symbols: List[SymbolInfo]
    ):
    ...
    def _extract_python_call(
        self, node: ASTNode, relationships: List[RelationshipInfo]
    ):
                ...
                if child.type == "formal_parameter":
                    ...
                    if name_node:
                        param = ParameterInfo(
                            name=self._get_node_text(name_node),
                            type_hint=(
                                self._get_node_text(type_node) if type_node else None
                            ),
                        )
                        params.append(param)
        return params
        ...
        if params_node:
            for child in params_node.children:
                if child.type == "parameter_declaration":
                    ...
                    if name_node:
                        params.append(
                            ParameterInfo(name=self._get_node_text(name_node))
                        )
        return params
