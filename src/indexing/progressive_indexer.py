...
class ProgressiveIndexer:
    ...
    def add_task(
        self, file_path: str, priority: IndexingPriority = IndexingPriority.NORMAL
    ):
        """
        ...
        try:
            ...
            if load_factor < 0.5:
                ...
                self.current_batch_size = min(
                    self.current_batch_size + 1, self.max_batch_size
                )
            elif load_factor > 0.8:
                # System under load, decrease batch size
                self.current_batch_size = max(
                    self.current_batch_size - 1, self.min_batch_size
                )
